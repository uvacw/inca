...
class bsch(Scraper):
    ...
    def process_links(self, links):
        for link in links:
            ...
            try:
                ...
                try:
                    text=" ".join(tree.xpath('//*[@class="bloque_Wtexto webedit"]/p//text()'))
                except:
    ...
    def get(self):
        ...
        tree = fromstring(requests.get(current_url).text.replace("csstg", "csgs").replace("http://staging.aacc.gs.corp", self.BASE_URL ))
        ...
        year_links = [self.BASE_URL+l.attrib['href'] for l in yearobjects if 'href' in l.attrib]
      
        for year_link in year_links:
            tree = (fromstring(requests.get(year_link).text.replace("csstg", "csgs")

...
class facebook(Scraper):
    ...
    def __init__(self,database=True, maxpages = 2, pagename = "groenlinks", app_id = "appID", app_secret = "appSecret"):
        '''
	...
	#retrieve the ID of the facebook page (needed for scraping)
        ...
        facebook_id = driver.find_element_by_xpath('//*[@id = "success-wrap"]/code').text
        driver.quit()
	...
	#construct the first  URL string
        page  = 1
        num_posts = 100
        fields = "/posts/?fields=message,link,created_time,type,name,id," + \
            "comments.limit(0).summary(true),shares,reactions" + \
            ".limit(0).summary(true)&limit="
        current_url = self.START_URL + str(facebook_id) + fields + str(num_posts) + '&access_token=' + access_token
        posts = []
        has_next_page = True
        first_page_text=""

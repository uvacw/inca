    ...
    def get(self,**kwargs):
        ...
        for link,date in zip(article_urls,dates): # you go to each article page       
            ...
            if tree.xpath("boolean(//*[@class='storytellingPage'])"):
                ...
                try:
                    summary = tree.xpath("//*[@class='text opener']/p/text()")[0].strip()
                except:
                    summary = ''
                ...
                if link.startswith('http://www.faz.net'):
                    matchObj = re.match(r'http://www.faz.net/(.*?)/(.*?)/(.*?)/', link, re.M|re.I)
                ...
                try:
                    source = tree.xpath("//*[@class='storyTmplBottomNav']//*[@class='quelle']/text()")[0].replace('Quelle: ','').strip()
                except:
                    source = ''
                    
                # Retrieving the byline/author
                try:
                    author = tree.xpath("//*[@class='over ']/small/text()")[0].replace('Von ','').title()
                except:
                    author = ''
                ...
                if tree.xpath("boolean(//*[@id='ArticlePagerBottom']/a[2]/@title)"):
                    ...
                    parag2 = tree2.xpath("//*[@class='FAZArtikelText']/div[3]/p//text()")
                    for r in parag2:
                        text += ' '+r.strip().replace('\xa0',' ')
                ...
                # Retrieving summary on top of articles
                try:
                    summary = tree.xpath("//*[@id='artikelEinleitung']/p/text()")[0].strip()
                except:
                    summary = ''
                ...
                # Retrieving the byline_source/source from url
                try:
                    source = tree.xpath("//*[@class='ArtikelFooter']/p//*[@class='quelle']/text()")[0].replace('Quelle: ','').strip()
                except:
                    source = ''
                    
                # Retrieving the byline/author
                try:
                    author = tree.xpath("//*[@id='artikelEinleitung']//*[@class='Autor']/span/a/span/text()")[0]
                except:
                    author = ''
                ...
                # get title
                try:
                    title = tree.xpath("//*[@id='artikelEinleitung']/h2/text()")[-1].strip()
                except:
                    title = ''

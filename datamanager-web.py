...
config.read('/home/damian/infra/config.conf')
...
<H1>Hoi!</H1>
<p>
Dit is een eerste testversie voor een infra-webinterface
</p>
''')
    overview1 = collectioncleanedNJR.aggregate([{"$group": {"_id": "$source", "number": {"$sum": 1}}}])
    print '<h2>Dit zit in onze database:</h2>'
    for combi in overview1["result"]:
        print combi["_id"], "\t", combi["number"],"<br>"
    print "<p>It's now",datetime.datetime.now(),"</p>"
    print '</body></html>' 

...
class twitter(Client):
    ...
    def add_application(self, appname="default"):
        ...
        app_prompt = {
            ...
            "description": "Please go to https://apps.twitter.com and click on 'Create New App' \n"
    ...
    def _set_delay(self, *args, **kwargs):
        now = time.time()
        earliest = self.load_credentials(
            app=kwargs.get("app", "default"), update_last_loaded=False
        )  ## MANUAL WORKAROUND - NEEDS INSPECTION
        if earliest:
    ...
    def get(
        ...
        try:
            self.update_credentials(
                credentials["_id"], **api.get_application_rate_limit_status()
            )
        except TwythonRateLimitError:
            pass  # sometimes you just can't get a rate-limit estimate

        if not force:
            since_id = doctype_first(
                doctype="tweets", query="user.screen_name:" + screen_name
            )
            if len(since_id) == 0:
                logger.info(
            ...
            while batchsize:
                ...
                for num, tweet in enumerate(tweets):
                    if self._check_exists(tweet["id_str"])[0] and not force:
                        logger.info(
                            "skipping existing {screen_name}-{tweet[id]}".format(
                                **locals()
                            )
                        )
                        continue
...
class twitter_followers(twitter):
    ...
    def get(self, *args, **kwargs):
        ...
        if not force:
            cursor = doctype_last(
                doctype="twitter_followers", query="user_screen_name:" + screen_name
            )
            if len(cursor) == 0:
                logger.debug(
                    "settings cursor to None as there are no followers for this user"
                )
                cursor = None
            else:
                cursor = cursor[0].get("_source", {}).get("cursor", None)
                logger.debug(
                    "force not requested, settings cursor to {cursor}".format(
                        **locals()
                    )
                )
            pass
        ...
        try:
            ...
            while batchsize > 0:
                ...
                logger.info(
                    "retrieved {batchsize} followers for {screen_name} in round {counter} out of {expected_rounds} expected rounds".format(
                        **locals()
                    )
                )
                counter += 1
            ...
            logger.debug(
                "expended credentials at cursur {cursor}".format(cursor=cursor)
            )
            try:
                self.update_credentials(
                    credentials["_id"], **api.get_application_rate_limit_status()
...
class twitter_users_lookup(twitter):
    ...
    def get(self, credentials, screen_names, force=False):
        ...
        try:
            ...
            batches = [
                screen_names[x : x + 100] for x in range(0, len(screen_names), 100)
            ]
            ...
            for batch in batches:
                ...
                for num, user in enumerate(users):
                    if self._check_exists(user["id_str"])[0] and not force:
                        logger.info(
                            "skipping existing {user[screen_name]} - {user[id]}".format(
                                **locals()
                            )
                        )
                        continue
                    ...
                    logger.info(
                        "retrieved profile for {user[screen_name]} with id {user[_id]}".format(
                            **locals()
                        )
                    )

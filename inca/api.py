...
def show_options(category):
    ...
    if not result:
        return json.dumps({'Results':'INCA currently provides these endpoints: %s' %', '.join(endpoints.keys())})
    else:
        tasks = [task[:-3] for task in os.listdir(category) if category[:-1] in task and task[-3:]==".py" ]
        return json.dumps({'Results' :'{category} provides these tasks: {tasks}'.format(**locals())})

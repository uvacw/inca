...
class hostelworld(Scraper):
    ...
    def get(self):
        ...
        page=1
        current_url = self.STARTURL+'?page='+str(page)
        ...
        while overview_page.text!=first_page_text:
            ...
            linkselement = tree.xpath('//*[@class="moreinfo button tiny radius hwta-property-link"]')
            links = [e.attrib['href'] for e in linkselement if 'href' in e.attrib]
            names = tree.xpath('//*[@class="resultheader rounded clearfix small-12 medium-7 large-8 columns clearfix"]/h2/a/text()')
            ratings = tree.xpath('//*[@class="hwta-rating-score"]/text()')
            reviewsquantity =  tree.xpath('//*[@class="hwta-rating-counter"]/text()')[1::2]
            prices = tree.xpath('//*[@class="price"]/text()')
            accommodationtypes = tree.xpath('//*[@class="proptype featureline"]//text()')
            accommodationtypesstrip = [x.strip() for x in accommodationtypes]

            assert len(links)==len(names)==len(ratings)==len(reviewsquantity)==len(prices)==len(accommodationtypesstrip)

            for i in range(len(links)):
                thishostel = {'name':names[i].strip(),
                              'link':links[i].strip(),
        ...
        for hostel in hostels:
            ...
            try:
                ratingdetail = tree.xpath('//*[@class="hwta-rating-text-score"]/text()')[0]
            except:
                ratingdetail = ""
            try:

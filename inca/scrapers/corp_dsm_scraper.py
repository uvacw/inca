...
class dsm(Scraper):
    ...
    def get(self, save):
        ...
        year = 2016
        current_url = self.START_URL+'limit.10.markets.dsm.offset.0.year.'+str(year)+'.html'
        ...
        while overview_page.text.find('info-item clearfix collapsed') != -1:
            ...
            while overview_page.text.find('info-item clearfix collapsed') != -1:
                ...
                for link in links:
                    ...
                    try:
                        title=" ".join(tree.xpath('//*[@class="title subtitle"]/h1/text()'))
                    except:
                        print("no title")
                        title = ""
                    try:
                        teaser=" ".join(tree.xpath('//*/span[@class="intro"]//text()'))
                    except:
                        teaser= ""
                        teaser_clean = " ".join(teaser.split())
                    try:
                        text=" ".join(tree.xpath('//*[@class="text parbase section"]/p//text()'))
                    except:
                ...
                page+=10
                current_url = self.START_URL+'offset.'+str(page)+'.year.'+str(year)+'.limit.10.html'
                overview_page = requests.get(current_url)

...
class Document(Task):
    ...
    functiontype = (
        ""
    )  # RSSscraper, text_processor, cluster_analysis, testing or other description of function
    ...
    def _save_document(self, document, forced=False):
        ...
        if type(document) == list:
            ...
            assert (
                self.doctype
            ), "You need to declare a `self.doctype` in your subclass!"
            assert (
                self.version
            ), "You need to declare a `self.version` in your subclass!"
            assert (
                self.functiontype
            ), "You need to declare a `self.functiontype` in your subclass!"
    ...
    def _add_metadata(self, document, **kwargs):
        ...
        try:
            docstring = self.get.__doc__
        except:
            try:
                docstring = self.process.__doc__
            except:
    ...
    def _check_complete(self):
        ...
        for attribute in ["functiontype", "version", "date", "doctype"]:
            if not getattr(self, attribute):
                logger.warning(
                    """%s misses the appropriate `%s` property!
        ...
        for method in ["process", "get"]:
            try:
                if getattr(self, method).__doc__ in teststrings:
                    logger.warning(
                        """%s's %s docstring does not reflect functionality!
                    Please update the docstring in your class definition."""

...
class Scraper(Document):
        ...
        if save == True:
            for doc in self.get(save, *args, **kwargs):
                if (
                    ...
                    logger.info(
                        "A document with this URL already existed - did not save the new one."
                    )
        else:
    ...
    def _test_function(self):
        ...
        try:
            self.check_exists = (
                lambda x: False
            )  # overwrite check exists to ensure start conditions
            for doc in self.get():
...
class UnparsableException(Exception):
    def __init__(self):
        logger.warn(
            "Could not parse the content; maybe the string does not contain valid HTML?"
        )

...
def insert_lexisnexis(pathwithlnfiles, recursive):
    ...
    if recursive:
        ...
        alleinputbestanden = [
            join(pathwithlnfiles, f)
            for f in listdir(pathwithlnfiles)
            if isfile(join(pathwithlnfiles, f)) and splitext(f)[1].lower() == ".txt"
        ]
    ...
    for bestand in alleinputbestanden:
        ...
        with open(bestand, "r", encoding="utf-8", errors="replace") as f:
            ...
            for line in f:
                ...
                elif line.lstrip().startswith("Copyright ") or line.lstrip().startswith(
                    "All Rights Reserved"
                ):
                    pass
                elif (
    ...
    if (
        ...
        == len(tekst)
        == len(pubdate_year)
        == len(pubdate_dayofweek)
        == len(pubdate_day)
        == len(pubdate_month)
    ):
        print("!!!!!!!!!!!!!!!!!!!!!!!!!")
        print(
            "Ooooops! Not all articles seem to have data for each field. These are the numbers of fields that where correctly coded (and, of course, they should be equal to the number of articles, which they aren't in all cases."
        )
        ...
        print()
        print(
            "Anyhow, we're gonna proceed and set those invalid fields to 'NA'. However, you should be aware of this when analyzing your data!"
        )
    ...
    for i in range(artikel):
        ...
        art = {
            ...
            "datum": datetime.datetime(
                int(art_pubdate_year), int(art_pubdate_month), int(art_pubdate_day)
            ),
    ...
    print(
        "\nInserted",
        len(tekst),
        "articles, out of which",
        suspicious,
        "might not be real articles, but, e.g., lists of stock shares. ",
    )
...
def insert_newsreleases(pathwithlnfiles, recursive):
    ...
    for i in range(artikel):
        ...
        if i + 1 in pubdate_hour.keys():
            ...
            art_pubdate = datetime.datetime(
                pubdate_year[i + 1], pubdate_month[i + 1], pubdate_day[i + 1]
            )
...
def adhocclean(bestand):
    ...
    for art in allarticles:
        ...
        for pat in replpatterns:
            subst = pat.subn(
                repldict[pat.pattern[2:-2]], thisart
            )  # [2:-2] to strip the \b
            thisart = subst[0]
...
def clean_database():
    ...
    print(
        "{}/{} articles have not been cleaned yet and will be cleaned".format(
            aantal, aantal2
        )
    )
    i = 0
    for art in allarticles:
        ...
        article_id = collection.update_one(
            {"_id": art["_id"]}, {"$set": {"textclean": thisart}}
        )
...
def clean_database_njr():
    ...
    for art in allarticles:
        ...
        for zin in parse(thisartorig).split():
            for token in zin:
                if (
                    token[1].startswith("N")
                    or token[1].startswith("J")
...
def main():
    ...
    group.add_argument(
        "--overview",
        help="Give an overview of the data stored in the collection",
        action="store_true",
    )
    group.add_argument(
        "--overview_cleaned",
        help="Give an overview of the data stored in the cleaned collection",
        action="store_true",
    )
    group.add_argument(
        "--overview_cleanednjr",
        help="Give an overview of the data stored in the cleaned collection of nouns, adjectives, and adverbs",
        action="store_true",
    )
    group.add_argument(
        "--insert_ln",
    ...
    group.add_argument(
        "--delete_all",
        help="Deletes everything in your database (!!!)",
        action="store_true",
    )
    parser.add_argument(
        "--recursive",
        help="Indicates that all subfolders are processed as well",
        action="store_true",
    )
    ...
    if args.delete_all:
        print(
            "Do you REALLY want to ERASE the whole collection",
            collectionname,
            "within the database",
            databasename,
            "?",
    ...
    if args.overview:
        ...
        overview1 = collection.aggregate(
            [{"$group": {"_id": "$source", "number": {"$sum": 1}}}]
        )
    ...
    if args.clean:
        print(
            "Do you REALLY want to clean the whole collection",
            collectionname,
            "within the database",
            databasename,
            "right now? This can take VERY long, and you might consider doing this overnight.",
        )
    ...
    if args.adhocclean:
        ...
        print(
            "Re-processing the cleaned database with the instructions provided in",
            bestand,
            "\n",
        )
        adhocclean(bestand)
